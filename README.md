...
| 文字 Agent 与 SSE | 已接 Qwen、BullMQ、真实高德 POI、PostgreSQL 单一事件源与 Android 断线续传 | 完整任务补充/取消端侧交互、状态机压力测试 |
...
- 高德 Web 服务用于真实 POI 查询，Android SDK 使用 `RouteSearchV2` 展示普通步行路线。高德当前没有轮椅路线规划模式，因此所有路线固定标注“普通步行路线（未验证无障碍）”；阶梯、扶梯、直梯、斜坡等 `roadType` 仅作为需现场复核的结构提示，不生成轮椅友好分
...
- 在真实 Voice Gateway 接通前，Android 不展示“发送至百炼”的 ASR/TTS 使用时同意弹窗，避免系统语音降级链被错误描述为百炼处理；接通后必须先取得相应能力的有效同意
- Agent 进度只通过 HTTP/SSE 传输，PostgreSQL `task_events` 是唯一事实源；Android 以 `Last-Event-ID` 和 `after` 双游标恢复，严格去重并忽略未知版本/事件，非终态断线按 1 到 30 秒退避重连
- SSE 游标必须是安全非负整数且属于当前任务；24 小时恢复窗口外、无效历史游标或缺失终态事件的终态快照会收到 `stream.reset`，客户端据此刷新任务快照并从最新事件继续或停止监听，不会把 Agent 状态混入语音 WebSocket
- SSE 开流同时受 Nginx 来源频率/并发和 Redis 安装账户/全局频率及短租约并发保护；Android 合并高频事件后读取最新快照，只有事件成功进入处理队列才推进游标，终态快照读取失败会继续退避重试
- 任务状态迁移与对应事件在同一个 PostgreSQL 事务中提交；Worker 用数据库 `run_claim_token` 做 fencing，只有当前声明者能清空/重建派生数据、写卡片或结束任务。Worker 被中断后，BullMQ 的同一 job 重投可以覆盖残留声明并幂等重建，旧 processor 恢复后无法再写，避免事件缺失、任务永久卡住或已完成卡片被旧进程删除
...
地图页在用户阅读高德第三方处理者、处理信息、用途和政策链接并同意之前，不初始化地图、搜索、路线或定位 SDK。地图与 Search SDK 均显式使用 HTTPS；一次性当前位置只驻留地图页内存，用户也可以拒绝定位并从真实地点搜索结果手动选择起点。高德 App 不可用时客户端会按服务端白名单动作自动尝试 HTTPS 网页；网页也不可用时提示用户点击“复制无障碍沟通卡”，不会静默写入剪贴板。客户端同时校验 scheme、host、userinfo 和端口，拒绝非白名单跳转。
